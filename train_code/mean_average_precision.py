...
def compute_average_precision(ranked_targets: np.ndarray, gtp: int) -> float:
    ...
    out = [
        compute_precision_at_k(ranked_targets, k + 1)
        for k in range(ranked_targets.size)
        if ranked_targets[k]
    ] 
    if len(out) == 0:
...
def calculate_map(
    ranked_retrieval_results: np.ndarray,
    ...
    assert ranked_retrieval_results.shape[1] == 500
    ...
    for gallery_indices, query_class_id in tqdm(
        zip(ranked_retrieval_results, query_labels), total=len(query_labels)
    ):
        ...
        assert len(np.unique(gallery_indices)) == len(gallery_indices), ValueError(
            "Repeated images in retrieval results"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(prog="mAP", description="mAP calculation")
    parser.add_argument(
        "--retrieval_result",
        type=str,
        required=True,
    ...
    ranked_retrieval_results = np.load(args.retrieval_result)[:, :500]
    mean_average_precision = calculate_map(
        ranked_retrieval_results, query_labels, gallery_labels
    )
